...
let servicios = [];
let editandoId = null;
const miUsuarioId = 7; // ✅ Tu ID como usuario registrado

async function cargarServicios() {
  const res = await fetch('/api/servicios');
  const todosLosServicios = await res.json();
  servicios = todosLosServicios.filter(s => s.usuario_id === miUsuarioId); // ⚠️ Filtrado
  renderizarServicios();
}
...
document.getElementById('service-form').addEventListener('submit', async function (e) {
  ...
  formData.append('usuario_id', miUsuarioId); // ✅ Asociar el servicio contigo
      ...
      <div class="service-card">
        ...
        <div class="service-info">
          ...
          <div class="service-actions">
            <button type="button" onclick="editarServicio(${serv.id})">✏️ Editar</button>
            <button type="button" onclick="eliminarServicio(${serv.id})">🗑️ Eliminar</button>
          </div>
        </div>
      </div>
    `;
